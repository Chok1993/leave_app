...
def _drop_drive_session() -> None:
    """ทิ้ง Drive service ใน session_state เพื่อสร้างใหม่รอบต่อไป"""
    st.session_state.pop("_drive_svc", None)
    logger.warning("Drive service reconnecting...")

def _drive_execute(request, retries: int = 4):
    """
    Execute Drive API request พร้อม retry
    ⚠️ request object ต้องถูกสร้างใหม่ทุกครั้งที่ reconnect
       เพราะ httplib2 connection ที่พังแล้วทำให้ segfault ถ้าใช้ request เดิม
    ดังนั้น caller ต้องส่ง callable มาแทน เพื่อให้ build request ใหม่ได้
    รองรับทั้ง 2 แบบ: request object (เดิม) และ callable (ใหม่)
    """
    _TE = (
    ...
    for attempt in range(retries):
        try:
            ...
            _drop_drive_session()   # ← ทิ้ง connection เดิม สร้างใหม่รอบต่อไป
...
def get_file_id(filename: str, parent_id: str = FOLDER_ID) -> Optional[str]:
    try:
        ...
        res = _drive_execute(lambda: svc.files().list(q=f"name='{filename}' and '{parent_id}' in parents and trashed=false", fields="files(id,modifiedTime)", orderBy="modifiedTime desc", supportsAllDrives=True, includeItemsFromAllDrives=True))
        files = res.get("files", [])
        if not files: return None
        keep_id = files[0]["id"]
        for dup in files[1:]:
            try: _drive_execute(lambda: svc.files().delete(fileId=dup["id"], supportsAllDrives=True))
...
def get_or_create_folder(folder_name: str, parent_id: str) -> Optional[str]:
    try:
        svc = get_drive_service()
        res = _drive_execute(lambda: svc.files().list(q=f"name='{folder_name}' and '{parent_id}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false", fields="files(id)", supportsAllDrives=True, includeItemsFromAllDrives=True))
        folders = res.get("files", [])
        if folders: return folders[0]["id"]
        new = _drive_execute(lambda: svc.files().create(body={"name":folder_name,"parents":[parent_id],"mimeType":"application/vnd.google-apps.folder"}, supportsAllDrives=True, fields="id"))
        return new.get("id")
...
def write_excel_to_drive(filename: str, df: pd.DataFrame, known_file_id: Optional[str] = None) -> bool:
    try:
        ...
        if fid: _drive_execute(lambda: svc.files().update(fileId=fid, media_body=media, supportsAllDrives=True))
        else: _drive_execute(lambda: svc.files().create(body={"name":filename,"parents":[FOLDER_ID]}, media_body=media, supportsAllDrives=True, fields="id"))
        st.cache_data.clear(); _invalidate_cache(); return True
    except Exception as e: logger.error(f"write_excel_to_drive({filename}): {e}"); st.error(f"บันทึกไฟล์ล้มเหลว: {e}"); return False
...
def backup_excel(filename: str, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        if existing:
            try: _drive_execute(lambda: svc.files().delete(fileId=existing, supportsAllDrives=True))
            except Exception: pass
        _drive_execute(lambda: svc.files().copy(fileId=fid, body={"name":bak_name,"parents":[bak_sub]}, supportsAllDrives=True))
    except Exception as e: logger.warning(f"backup_excel({filename}): {e}")
...
def upload_pdf_to_drive(uploaded_file, new_filename: str, folder_id: str) -> str:
    try:
        ...
        created = _drive_execute(lambda: svc.files().create(body=meta, media_body=media, supportsAllDrives=True, fields="id,webViewLink"))
...
def _load_all_data_to_cache(force: bool = False) -> None:
    ...
    with st.spinner("⏳ กำลังโหลดข้อมูล..."):
        ...
        df_travel_raw,_fid_travel=read_excel_with_backup(FILE_TRAVEL,dedup_cols=["ชื่อ-สกุล","วันที่เริ่ม","เรื่อง/กิจกรรม"])
        df_staff,_fid_staff=read_excel_with_backup(FILE_STAFF,dedup_cols=["ชื่อ-สกุล"])
        df_att=read_attendance_report(); df_manual=load_manual_scans(); df_travel_all=load_all_travel()
        df_leave,df_travel_raw,df_att=preprocess_dataframes(df_leave,df_travel_raw,df_att)
        ...
        st.session_state.update({"cache_leave":df_leave,"cache_travel":df_travel_raw,"cache_travel_all":df_travel_all,"cache_att":df_att,"cache_staff":df_staff,"cache_manual":df_manual,"_fid_leave":_fid_leave,"_fid_travel":_fid_travel,"_fid_staff":_fid_staff,"_data_loaded_at":dt.datetime.now()})
