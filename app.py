...
# ----------------------------
# 📅 การมาปฏิบัติงาน (เวอร์ชันใหม่ ตรวจไขว้ 3 ฐานข้อมูล + เวลาจริง)
# ----------------------------
    ...
    if df_att.empty:
        st.warning("⚠️ ยังไม่มีข้อมูลสแกนเข้า-ออกในระบบ หรือไม่พบไฟล์ attendance_report.xlsx")
        st.stop()
    ...
    # ✅ แปลงวันที่ให้เป็น datetime ทั้ง 3 ไฟล์
    ...
    for name in df_month[name_col].unique():
        for d in date_range:
            ...
            # ✅ ถ้าไม่มีข้อมูลสแกน ให้ตรวจสอบลาและราชการ
            else:
                in_leave = (
    ...
    # ✅ ดาวน์โหลดรายงานรายวัน
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df_daily.to_excel(writer, index=False, sheet_name="รายงานรายวัน")
    output.seek(0)
    st.download_button(
