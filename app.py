...
# ===================================================
# 📅 การมาปฏิบัติงาน (เวอร์ชันสมบูรณ์ 2025)
# ===================================================
    ...
    # ✅ ตรวจว่ามีข้อมูลสแกนหรือไม่
    if df_att.empty:
        st.warning("⚠️ ยังไม่มีข้อมูลสแกนเข้า-ออกในระบบ หรือไม่พบไฟล์ scan_report.xlsx")
        st.stop()

    # ✅ ตรวจหาคอลัมน์ชื่อบุคลากร
    name_col = None
    for possible in ["ชื่อพนักงาน", "ชื่อ-สกุล", "ชื่อ"]:
        if possible in df_att.columns:
            name_col = possible
            break
    if not name_col:
        st.error("❌ ไม่พบคอลัมน์ชื่อบุคลากร (เช่น 'ชื่อพนักงาน' หรือ 'ชื่อ-สกุล')")
        st.stop()
    ...
    if len(months) == 0:
        st.warning("⚠️ ไม่มีข้อมูลวันที่ในไฟล์สแกน")
        st.stop()

    # 🔹 กำหนดเดือนล่าสุดอัตโนมัติ
    latest_month = months[-1]

    # 🔸 แสดง selectbox โดยเลือกเดือนล่าสุดเป็นค่าเริ่มต้น
    selected_month = st.selectbox(
        "📆 เลือกเดือนที่ต้องการดู",
        months,
        index=months.index(latest_month),
        help=f"เดือนล่าสุดที่มีข้อมูลคือ {latest_month}"
    )

    # 🧭 แจ้งเตือนกรณีเลือกเดือนที่ไม่มีข้อมูล
    if selected_month != latest_month:
        st.info(f"ℹ️ คุณเลือกเดือน {selected_month} ซึ่งอาจไม่มีข้อมูลในไฟล์ — เดือนล่าสุดคือ {latest_month}")

    # 🧍‍♀️ เลือกชื่อบุคลากร
    selected_names = st.multiselect(
        "👤 เลือกชื่อบุคลากร",
        sorted(df_att[name_col].dropna().unique()),
        max_selections=5
    )

    # ✅ กรองเฉพาะเดือนและชื่อที่เลือก
    ...
    for name in df_month[name_col].unique():
        for d in date_range:
            ...
            # 🔹 ตรวจว่ามีการสแกนวันนั้นไหม
            att = df_month[
                (df_month[name_col] == name)
                & (df_month["วันที่"].dt.normalize() == pd.to_datetime(d))
            ]

            if not att.empty:
                rec["เวลาเข้า"] = str(att.iloc[0].get("เวลาเข้า", "")).strip()
                rec["เวลาออก"] = str(att.iloc[0].get("เวลาออก", "")).strip()
                rec["หมายเหตุ"] = str(att.iloc[0].get("หมายเหตุ", "")).strip()
                ...
                if d.weekday() >= 5:
                    ...
                    try:
                        t_in = pd.to_datetime(rec["เวลาเข้า"], format="%H:%M").time() if rec["เวลาเข้า"] else None
                        t_out = pd.to_datetime(rec["เวลาออก"], format="%H:%M").time() if rec["เวลาออก"] else None
                    except:
                        t_in, t_out = None, None

                    # 🔹 ตรวจสอบสถานะเวลาเข้าออก
    ...
    # --------------------------------------------------
    st.markdown("---")
    st.subheader("📈 แนวโน้มการมาปฏิบัติงานในเดือนนี้")

    daily_trend = df_daily.groupby(["วันที่", "สถานะ"]).size().reset_index(name="จำนวนคน")
    ...
    if not daily_trend.empty:
        chart = (
            ...
            .encode(
                ...
                y=alt.Y("จำนวนคน:Q", title="จำนวนบุคลากร"),
                color=alt.Color("สถานะ:N", title="สถานะ")
            )
            .properties(width=900, height=400)
        )
        st.altair_chart(chart, use_container_width=True)
    else:
        st.info("ไม่มีข้อมูลแนวโน้มในเดือนนี้")

    # --------------------------------------------------
    # 🥧 กราฟภาพรวมสัดส่วนสถานะ
    # --------------------------------------------------
    st.markdown("---")
    st.subheader("🥧 ภาพรวมสัดส่วนการมาปฏิบัติงานทั้งเดือน")

    pie_data = (
        df_daily["สถานะ"]
        .apply(lambda s: "ลา" if str(s).startswith("ลา") else s)
        .value_counts()
        .reset_index()
    )
    pie_data.columns = ["สถานะ", "จำนวนวัน"]
    ...
    if not pie_data.empty:
        pie_chart = (
            ...
            .encode(
                theta=alt.Theta("จำนวนวัน:Q", title="จำนวนวัน"),
                color=alt.Color("สถานะ:N", title="สถานะ"),
                tooltip=["สถานะ", "จำนวนวัน"]
            )
            .properties(width=500, height=400)
        )
        st.altair_chart(pie_chart, use_container_width=True)
    else:
        st.info("ไม่มีข้อมูลสำหรับแสดงกราฟภาพรวม")
