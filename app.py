...
def _drive_execute(request, retries: int = 4):
    ...
    for attempt in range(retries):
        try:
            ...
            if status in (429,500,502,503,504): time.sleep((2**attempt)+0.5); last_exc=e; continue
            raise
        except _TE as e:
            logger.warning(f"Transport error retry {attempt+1}/{retries}: {type(e).__name__}")
            time.sleep(2**attempt); last_exc=e; continue
        except Exception as e:
...
def get_file_id(filename: str, parent_id: str = FOLDER_ID) -> Optional[str]:
    try:
        ...
        res = _drive_execute(svc.files().list(q=f"name='{filename}' and '{parent_id}' in parents and trashed=false", fields="files(id,modifiedTime)", orderBy="modifiedTime desc", supportsAllDrives=True, includeItemsFromAllDrives=True))
        files = res.get("files", [])
        if not files: return None
        keep_id = files[0]["id"]
        for dup in files[1:]:
            try: _drive_execute(svc.files().delete(fileId=dup["id"], supportsAllDrives=True))
...
def get_or_create_folder(folder_name: str, parent_id: str) -> Optional[str]:
    try:
        svc = get_drive_service()
        res = _drive_execute(svc.files().list(q=f"name='{folder_name}' and '{parent_id}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false", fields="files(id)", supportsAllDrives=True, includeItemsFromAllDrives=True))
        folders = res.get("files", [])
        if folders: return folders[0]["id"]
        new = _drive_execute(svc.files().create(body={"name":folder_name,"parents":[parent_id],"mimeType":"application/vnd.google-apps.folder"}, supportsAllDrives=True, fields="id"))
        return new.get("id")
