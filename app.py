...
@st.cache_data(ttl=600)
def read_excel_from_drive(filename: str) -> pd.DataFrame:
    """อ่านไฟล์ Excel จาก Shared Drive; คืนค่า DataFrame หรือว่างหากหาไม่เจอ"""
    try:
        file_id = get_file_id(filename)
        if not file_id:
            st.warning(f"⚠️ ไม่พบไฟล์ {filename} ใน Shared Drive ที่กำหนดไว้")
            return pd.DataFrame()
        ...
        # 🧩 โหลดชีตแรกเสมอ (ไม่สนชื่อ)
        xls = pd.ExcelFile(fh, engine="openpyxl")
        df = pd.read_excel(xls, sheet_name=xls.sheet_names[0])

        # 🩹 ถ้าชื่อคอลัมน์ไม่เจอ เช่น "Unnamed: 0"
        if "ชื่อพนักงาน" not in df.columns and "ชื่อ-สกุล" not in df.columns:
            df.columns = df.columns.str.strip()
            df = df.dropna(how="all")  # ลบแถวว่าง
            # เผื่อเผลอ header อยู่แถว 2
            if "ชื่อพนักงาน" not in df.columns and len(df) > 0:
